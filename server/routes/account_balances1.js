...
const your_account1_key = process.env.your_account1_key;
const your_account1_secret = process.env.your_account1_secret;

// Helper function to sign a query
function signQuery(query) {
  return crypto.createHmac('sha256', your_account1_secret).update(query).digest('hex');
}
...
async function getFuturesAccountDetails() {
  ...
  try {
    const response = await axios.get(`${endpoint}?${params.toString()}`, {
      headers: {
        'X-MBX-APIKEY': your_account1_key,
      },
    });
...
async function your_account1_balance_data() {
...
router.get('/', async (req, res) => {
  try {
    const { available, marginalBalance, totalWalletBalance, totalUnrealizedPnL } = await your_account1_balance_data();
...
module.exports.your_account1_balance_data = your_account1_balance_data; // For standalone function
...
// module.exports = {router, your_account1_balance_data};
