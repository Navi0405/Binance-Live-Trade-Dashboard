...
const your_account2_key = process.env.your_account2_key;
const your_account2_secret = process.env.your_account2_secret;

// Helper function to sign a query
function signQuery(query) {
  return crypto.createHmac('sha256', your_account2_secret).update(query).digest('hex');
}
...
async function getFuturesAccountDetails() {
  ...
  try {
    const response = await axios.get(`${endpoint}?${params.toString()}`, {
      headers: {
        'X-MBX-APIKEY': your_account2_key,
      },
    });
...
async function your_account2_balance_data() {
...
router.get('/', async (req, res) => {
  try {
    const { available, marginalBalance, totalWalletBalance, totalUnrealizedPnL } = await your_account2_balance_data();
...
module.exports.your_account2_balance_data = your_account2_balance_data; // For standalone function
...
// module.exports = {router, your_account2_balance_data};
